(function(root){
  ...
  function checkoutToken(){
    ...
    return url.searchParams.get('checkout')||readSession()?.token||'';
  }
  async function waitForCheckout(token){
    if(!token)return;
    const deadline=Date.now()+6*60*1000;
    toast('Проверяем заказ…');
    while(Date.now()<deadline){
      try{
        const r=await fetch(API+'/api/checkout/'+encodeURIComponent(token),{cache:'no-store'});
        const j=await r.json();
        if(r.ok&&j.status==='ORDER_CREATED'&&j.trackingToken){
          clearSession();
          saveTrackingToken(j.trackingToken);
          location.replace('/?order='+encodeURIComponent(j.trackingToken));
          return;
        }
  ...
  async function verifiedSubmitOrder(){
    ...
    try{
      const r=await fetch(API+'/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
      const j=await r.json();if(!r.ok)throw new Error(j.error||'Не удалось начать оформление');
      saveSession({token:j.checkoutToken,expiresAt:j.expiresAt});
  ...
  window.addEventListener('load',()=>{
    ...
    if(token)waitForCheckout(token);
  });
})(globalThis);
